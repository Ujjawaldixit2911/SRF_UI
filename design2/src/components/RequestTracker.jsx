...
export default function RequestTracker() {
  ...
  return (
    <div className="view active" id="view-tracker" style={{display: 'flex', gap: '24px', alignItems: 'flex-start', flexDirection: 'row'}}>
      ...
      <div className="rt-main" style={{flex: 1, minWidth: 0, display: 'flex', flexDirection: 'column', gap: '24px'}}>
        {/* Actionable */}
            ...
            <table className="rt">
              ...
              <tbody>
                {(expanded.act ? RT_ACTIONABLE : RT_ACTIONABLE.slice(0, 5)).map((r, i) => (
                  <tr key={i}>
                    <td>
                      <div className="td-name" style={{fontFamily: 'monospace'}}>{r[0]}</div>
                      <div className="td-text" style={{marginTop: '6px', fontSize: '11px', color: 'var(--blue)', fontWeight: '700'}}>
                        ASSIGNED TO: ME
                      </div>
                    </td>
                    <td><div className="td-name">{r[1]}</div></td>
                    <td><div style={{fontWeight: '600'}}>{r[4]}</div></td>
                    <td><span className={`st-badge ${stClass(r[3])}`}>{r[3]}</span></td>
                    <td>
                      <div style={{fontWeight: '600'}}>{r[6]}</div>
                      <div style={{fontSize: '11px', color: 'var(--muted)'}}>{r[5]}</div>
                    </td>
                    <td>
                      <div style={{fontWeight: '600'}}>{r[7]}</div>
                      <div style={{fontSize: '11px', color: 'var(--muted)'}}>{r[8] || '—'}</div>
                    </td>
                  </tr>
                ))}
              </tbody>
              ...
              <tbody>
                {(expanded.nonact ? RT_NONACT : RT_NONACT.slice(0, 5)).map((r, i) => (
                  <tr key={i}>
                    <td>
                      <div className="td-name" style={{fontFamily: 'monospace'}}>{r[0]}</div>
                      <div className="td-text" style={{marginTop: '6px', fontSize: '11px', color: 'var(--amber)', fontWeight: '700'}}>
                        ASSIGNED: {r[7]}
                      </div>
                    </td>
                    <td><div className="td-name">{r[1]}</div></td>
                    ...
                    <td><span className={`st-badge ${stClass(r[3])}`}>{r[3]}</span></td>
                    <td>
                      <div style={{fontWeight: '600'}}>{r[5]}</div>
                      <div style={{fontSize: '11px', color: 'var(--muted)'}}>{r[6]}</div>
                    </td>
                    <td>
                      <div style={{fontWeight: '600'}}>{r[9]}</div>
                      <div style={{fontSize: '11px', color: 'var(--muted)'}}>{r[10] || '—'}</div>
                    </td>
                  </tr>
                ))}
              </tbody>
              ...
              <tbody>
                {(expanded.drafts ? RT_DRAFTS : RT_DRAFTS.slice(0, 5)).map((r, i) => (
                  <tr key={i}>
                    <td>
                      <div className="td-name" style={{fontFamily: 'monospace'}}>{r[0]}</div>
                      <div className="td-text" style={{marginTop: '6px', fontSize: '11px', color: 'var(--muted)', fontWeight: '700'}}>
                        DRAFT
                      </div>
                    </td>
                    <td><div className="td-name">{r[1]}</div></td>
                    <td><div style={{fontWeight: '600'}}>{r[7]}</div></td>
                    ...
                    <td>
                      <div style={{fontWeight: '600'}}>{r[4]}</div>
                      <div style={{fontSize: '11px', color: 'var(--muted)'}}>{r[5] || '—'}</div>
                    </td>
                  </tr>
                ))}
